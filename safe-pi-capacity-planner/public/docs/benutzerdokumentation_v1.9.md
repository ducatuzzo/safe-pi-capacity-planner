...
Pro PI können beliebig viele **SAFe-Zeremonien** als kalendarische Termine erfasst werden — als Einzeltermine oder als Terminserien (Outlook-Style). Jede Zeremonie kann einzeln als `.ics`-Datei für Outlook/Apple/Google Calendar exportiert werden — Serien werden via RFC-5545-`RRULE` als ein Eintrag mit allen Wiederholungen übermittelt.

**Tabellen-Spalten (ab v1.9 / Schema 1.6):**
...
| Ende | `YYYY-MM-DD HH:MM` (kann am gleichen Tag oder später sein — mehrtägige Termine möglich) |
| Serie | `—` für Einzeltermine, sonst z.B. `↻ Wöchentlich × 5` oder `↻ Täglich (alle 2) bis 2026-12-31` |
| Iter. | Optionale Zuordnung zu einer Iteration |
| .ics | Download-Button (RFC-5545-konforme Datei mit RRULE bei Serien) |
| ✏ / 🗑 | Bearbeiten / Löschen |

**Modal — Felder:**

- **Typ**, **Titel**: wie bisher
- **Start-Datum**, **Start-Zeit**: Beginn des Termins
- **Ende-Datum**, **Ende-Zeit**: Ende des Termins (kann am gleichen oder einem späteren Tag liegen)
...
- **Ort / Link**, **Beschreibung**, **Iteration** (optional): wie bisher
...
> Zeremonien sind rein kalendarisch — sie ziehen **keine Kapazität ab** und blockieren keine Buchungen. Einzeltermine erscheinen als `◆`-Marker, Serien-Instanzen als `◈`-Marker im Planungs-Kalender (siehe Abschnitt «Kalender-Header»).
