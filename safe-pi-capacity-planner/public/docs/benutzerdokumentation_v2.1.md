...
| **Einstellungen** | Mitarbeiter, PI-Planung, Feiertage, **Custom Types**, Team-Konfiguration, Globale Parameter, Farben verwalten |
| **Admin** | Geschützter Bereich: Train-Verwaltung, Daten-Reset, Admin-Code ändern |

> **Mobile-Ansicht (ab v2.1):** Auf Smartphone/Tablet erscheint eine **Read-Only-Ansicht** mit Bottom-Tab-Bar (4 Icons: Planung / Kapazität / Dashboard / PI Dashboard). Settings/Admin und alle Schreib-Aktionen (Drag-Buchung, Undo/Redo, Excel-Import, Export-Buttons) sind ausgeblendet. Ein Banner oben informiert: „Mobile Ansicht — nur lesen. Für Bearbeitung bitte am Desktop öffnen."
...
### Rückgängig / Wiederherstellen (NEU v2.1)

Alle Änderungen an Buchungen UND Einstellungen können rückgängig gemacht werden — sowohl per Toolbar-Button als auch per Tastatur:

| Aktion | Tastatur | Toolbar-Button (Planung-Tab oben rechts) |
|---|---|---|
| **Rückgängig** | `Strg + Z` | ⟲ |
| **Wiederherstellen** | `Strg + Y` oder `Strg + Shift + Z` | ⟳ |

**Was wird rückgängig gemacht:**
- Drag-Buchungen einzelne Tage UND ganze Drag-Bereiche (eine Drag-Aktion = ein Undo-Schritt)
- Settings-Änderungen: Mitarbeiter, PI-Planung, Feiertage, Schulferien, Blocker, Team-Konfiguration, Globale Parameter, PI-Team-Ziele, Custom Allocation Types

**Stack-Tiefe:** 5 Schritte. Älteste Aktion fällt aus dem Stack.

**Hinweis:** Innerhalb von Eingabefeldern (Text-Inputs, Textareas) greift weiterhin der Browser-Undo — der App-Undo wird in diesem Kontext ignoriert.

### Excel-Clipboard-Import (NEU v2.1)

Du kannst Tagesbuchungen direkt aus Excel kopieren und in den Planungs-Kalender einfügen — ohne Datei-Upload:

1. In Excel die Zellen markieren (Datums-Zeile + Buchungstyp-Werte pro Mitarbeiter)
2. `Strg + C` (kopieren)
3. Im Planungs-Tab eine Mitarbeiter-Zelle klicken (= Einfüge-Ursprung markieren — orangener Rand)
4. `Strg + V` (einfügen) → Dialog erscheint mit Vorschau

**Erkennungs-Modi:**

| Modus | Erkennung | Beispiel-Input |
|---|---|---|
| **Structured** (mit Header) | Erste Zeile enthält Datumsangaben (YYYY-MM-DD, DD.MM.YYYY, ...) | `2026-04-27	2026-04-28	2026-04-29`<br>`F	F	A` |
| **Raw** (ohne Header) | Erste Zeile enthält Buchungstyp-Werte | `F	F	A	BP`<br>(wird ab der markierten Origin-Zelle eingefügt) |

**Akzeptierte Werte pro Zelle:**
- Buchungstyp-Kürzel: `F`, `A`, `T`, `M`, `I`, `B`, `BP`, `P`
- Voller Name (z.B. `FERIEN`, `PIKETT`)
- Custom-Type-Code (z.B. `IM`, `RE`)
- Leer (= keine Buchung)

**Vorschau-Dialog:**
- Konflikte mit bestehenden Buchungen werden markiert
- Du kannst **Überschreiben** (alte Buchung wird ersetzt) oder **Nur leere Zellen füllen** wählen
- Anzahl Zellen, betroffene Mitarbeiter, Zeitraum werden angezeigt
...
### Custom Allocation Types (NEU v2.1 — Feature 22)

**Pfad:** Einstellungen → Custom Types

Zusätzlich zu den 8 Standard-Buchungstypen (Ferien, Abwesend, Teilzeit, Militär, IPA, Betrieb, Betrieb+Pikett, Pikett) können beliebige **eigene Buchungstypen** definiert werden — z.B. „Innovationswoche", „Recurring Meetings", „Schulung".

| Spalte | Beschreibung |
|---|---|
| **Code** | 1–4-stelliges Kürzel (z.B. `IM`, `RE`, `SCH`) — wird in der Legende und beim Drag verwendet |
| **Label** | Anzeigename (z.B. „Innovationswoche") |
| **Kategorie** | Zuordnung zu einer der drei SP-Kategorien (siehe unten) |
| **Farbe** | Hintergrund-Hex (Color-Picker) |
| **Textfarbe** | Schrift-Hex (Color-Picker) |

**Drei Kategorien — bestimmen die SP-Berechnung:**

| Kategorie | SP-Wirkung | Beispiel-Use-Case |
|---|---|---|
| **ABSENCE** | 0 SP — wie Ferien/Abwesend | Innovationswoche, Workshop, Krankheit |
| **OPERATIONAL** | Betrieb-Lücken-Check | Recurring Maintenance, On-Call-ähnliche Aufgaben |
| **NONE** | Neutral — wirkt nicht auf SP-Berechnung | Markierung, Notiz, optionale Teilnahme |

**Bedienung:**
1. «+ Neu» — neuer Custom-Type-Eintrag
2. Code + Label + Kategorie + Farben ausfüllen
3. «Speichern» — sofort in allen Tabs verfügbar (Drag-Legende, Filter, Excel-Import)

**Limit:** maximal 20 Custom Types pro Train.

**Hinweis:** Custom Types werden ausschliesslich via Server-State synchronisiert — sie sind im JSON-Backup enthalten. Beim Drag erscheinen sie in der Buchungstyp-Legende rechts neben den Standard-Typen mit einem kleinen `★`-Symbol.
...
| **2.1** | **10.06.2026** | **Feature 22 (Custom Allocation Types): eigene Buchungstypen pro Train mit Code, Label, Kategorie (ABSENCE/OPERATIONAL/NONE) und Farben — wirken in Drag-Legende, SP-Berechnung, Filter und Excel-Import. Feature 27 (Mobile Read-Only Responsive Design): Smartphone/Tablet-optimierte Ansicht mit Bottom-Tab-Bar, ausgeblendeten Schreib-Aktionen, kompaktem Header, kollabierbarer Filterleiste und Mobile-Read-Only-Banner. Global Undo/Redo: Stack-Tiefe 5, deckt Drag-Buchungen UND alle Settings-Änderungen ab; Tastatur `Strg+Z` / `Strg+Y` / `Strg+Shift+Z`; Toolbar-Buttons ⟲/⟳ im Planung-Tab; Eingabefelder bleiben Browser-Undo überlassen. Excel-Clipboard-Import: Tagesbuchungen direkt aus Excel via `Strg+V` einfügen — Structured-Modus (mit Datums-Header) oder Raw-Modus (ohne Header, ab Einfüge-Origin); Vorschau-Dialog mit Konflikt-Markierung; Optionen «Überschreiben» / «Nur leere Zellen füllen». Admin-Code von 6 auf 8 Ziffern gehärtet (Default `00000815`), Recovery-Endpoint für Lockout. Löschen-Konsolidierung im Admin.** |
